...
/**
 * This class denotes a rule set for elements defined within a class which represents an originator.
 * */
public class OriginatorRuleSet implements RuleSet {
    ...
    public VerificationResult verifyElement(TypeElement annotation, Element elementToVerify, Element enclosingElement) {
        ...
        case VERIFY_SAVE_MEMENTO_STATE_METHOD:
            verificationResult = verifySaveMementoStateMethod(annotation, elementToVerify, enclosingElement);
            break;
        case VERIFY_RESTORE_MEMENTO_STATE_METHOD:
            verificationResult = verifySaveMementoStateMethod(annotation, elementToVerify, enclosingElement);
            break;
        case VERIFY_SET_STATE_METHOD:
            verificationResult = verifySetStateMethod(annotation, elementToVerify, enclosingElement);
            break;
        case VERIFY_STATE_FIELD:
            verificationResult = verifyStateVariable(annotation, elementToVerify, enclosingElement);
            break;
        default:
     ...
     * */
    private VerificationResult verifyStateVariable(TypeElement annotation, Element element, Element enclosingElement) {
        ...
        if (element.getModifiers().contains(Modifier.PUBLIC)) {
            return new VerificationResult(Diagnostic.Kind.WARNING, element.getKind(), element, "Variable declaration " +
                    "" + "" + "" + "" + elementName + ENCLOSING_ELEMENT + enclosingElementName + ANNOTATED_WITH + annotation.getSimpleName().toString() + " should not have " +
                    "public " + "" + "modifier.");
        } else {
     ...
     * @return VerificationResult The result of the verification.
     * */
    private VerificationResult verifySaveMementoStateMethod(TypeElement annotation, Element element, Element enclosingElement) {
     ...
     * @return VerificationResult The result of the verification.
     * */
    private VerificationResult verifySetStateMethod(TypeElement annotation, Element element, Element enclosingElement) {
